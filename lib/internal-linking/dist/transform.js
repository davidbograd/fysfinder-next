 ...
 */
function prepareKeywordMap(config) {
 ...
 */
export function processInternalLinks(html, config, currentPagePath) {
    const keywordMap = prepareKeywordMap(config);
    const linkedKeywordsSet = new Set(); // Keep track of keywords linked *on this page*
    const actuallyLinkedKeywords = []; // List of keywords we actually link
    ...
    processedHtml = processedHtml.replace(pTagRegex, (match, openingTag, pContent, closingTag) => {
