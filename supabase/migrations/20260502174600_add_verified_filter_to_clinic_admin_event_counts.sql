-- Migration: Add verified-only filtering to clinic-level admin analytics

DROP FUNCTION IF EXISTS get_clinic_admin_event_counts(
  TIMESTAMPTZ,
  TIMESTAMPTZ,
  INTEGER,
  INTEGER,
...
AS $$
  WITH clinic_counts AS (
    ...
    WHERE (p_start_date IS NULL OR ce.created_at >= p_start_date)
      ...
      AND (NOT p_verified_only OR c.verified_klinik = TRUE)
