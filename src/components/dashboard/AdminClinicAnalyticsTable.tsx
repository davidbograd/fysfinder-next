// Sortable clinic analytics table for the admin dashboard.
...
import type { ClinicAdminAnalyticsRow } from "@/app/actions/admin-stats";
...
interface AdminClinicAnalyticsTableProps {
  rows: ClinicAdminAnalyticsRow[];
}
...
export const AdminClinicAnalyticsTable = ({ rows }: AdminClinicAnalyticsTableProps) => {
  ...
  const sortedRows = useMemo(() => {
    return [...rows].sort((a, b) => {
      ...
      return a.clinicName.localeCompare(b.clinicName, "da");
    });
  ...
  if (rows.length === 0) {
    return <p className="text-sm text-gray-500">Ingen klinikdata endnu.</p>;
  }
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-white">
        <Table className="mt-0 [&_th]:border-x-0 [&_td]:border-x-0 [&_th]:bg-zinc-600 [&_tbody]:bg-white [&_tfoot]:bg-white">
          <TableHeader className="[&_tr]:border-b-zinc-500">
            <TableRow>
              <TableHead className="h-10 text-xs font-medium uppercase tracking-wide text-white">
                Klinik
              </TableHead>
              <TableHead className="h-10">
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  className="h-auto rounded-full px-0 py-0 text-xs font-medium uppercase tracking-wide text-white hover:bg-white/10 hover:text-white"
                ...
                column.parent === "leadClicks" && visibleBreakdownColumns[column.key] ? (
                  <TableHead key={column.key} className="h-10">
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      className="h-auto rounded-full px-0 py-0 text-[11px] font-normal uppercase tracking-wide text-white/70 hover:bg-transparent hover:text-white/85"
          ...
          <TableBody>
            <TableRow className="bg-zinc-50/80 font-medium">
              ...
              <TableCell className="font-semibold tabular-nums">
                {formatNumber(totals.leadClicks)}
              </TableCell>
              {BREAKDOWN_COLUMNS.map((column) =>
                column.parent === "leadClicks" && visibleBreakdownColumns[column.key] ? (
            ...
            {sortedRows.map((row) => (
              <TableRow key={row.clinicId}>
                <TableCell>
                  ...
                  {row.suburb ? (
                    <div className="text-xs text-gray-500">{row.suburb}</div>
                  ) : null}
                </TableCell>
                <TableCell className="font-semibold tabular-nums">
                  {formatNumber(row.leadClicks)}
                </TableCell>
                {BREAKDOWN_COLUMNS.map((column) =>
                  column.parent === "leadClicks" && visibleBreakdownColumns[column.key] ? (
