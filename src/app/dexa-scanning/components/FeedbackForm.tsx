...
export function FeedbackForm({ onReset }: FeedbackFormProps) {
  const handleFeedback = (isPositive: boolean) => {
    ...
    if (typeof window !== "undefined" && "gtag" in window) {
      (window as any).gtag("event", "dexa_scanning_feedback", {
        event_category: "DEXA Scanning",
    ...
    if (!isPositive) {
      window.open("https://tally.so/r/3EAOgA", "_blank");
    }
    ...
    <div className="space-y-4 p-4 sm:p-6">
      <div className="space-y-1">
        ...
        <p className="text-center font-bold text-gray-800">
          Forstår du din DEXA-scanning bedre?
        </p>
      </div>
