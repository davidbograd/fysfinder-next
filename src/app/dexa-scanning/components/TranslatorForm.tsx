...
export function TranslatorForm() {
  ...
  const loadingStates = [
    { message: "Læser DEXA scanning", duration: 2500 },
  ...
  const handleTranslate = async () => {
    if (!input.trim()) {
      setError("Indsæt tekst fra din DEXA-scanning rapport.");
      return;
    }

    if (input.length < 10) {
      setError("Teksten er for kort til at være en DEXA-scanning rapport.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/dexa-scanning", {
        method: "POST",
        headers: {
  ...
  const handleFeedback = (isPositive: boolean) => {
    ...
    if (typeof window !== "undefined" && "gtag" in window) {
      (window as any).gtag("event", "dexa_translation_feedback", {
        event_category: "DEXA Translation",
          ...
          {!translation ? (
            <>
              <div className="relative">
                <Textarea
                  placeholder="Indsæt teksten fra din DEXA-scanning her..."
                  value={input}
            ...
            <>
              <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md text-sm text-yellow-800">
                Dette er en AI-assisteret oversættelse af din DEXA-scanning
